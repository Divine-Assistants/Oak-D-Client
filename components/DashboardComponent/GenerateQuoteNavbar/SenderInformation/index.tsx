...
import { SenderInformationContext, ReceiverInformationContext, DomesticBreadcrumbContext } from '@/context/UserDashboardGenerateQuote';
import { senderInfoContext } from '@/context/PackageInfo';
import { WarehouseSenderInfoContext } from '@/context/Warehouse';
...
export function SenderInformation({setData}: SenderInformationProps){
    ...
    const {showWarehouseSenderInfo} = useContext(WarehouseSenderInfoContext);
    const {domesticBreadcrumb, setDomesticBreadcrumb} = useContext(DomesticBreadcrumbContext);

    const [senderData, setSenderData] = useState<userInfo>(initialUserInfo);
    ...
    return( 
        <section style={{display: !showWarehouseSenderInfo && showSenderInfo ? 'block' : 'none'}} className=" p-[20px]">
            <DomesticBreadcrumb />
            ...
            <div className='text-[24px] text-[#1E1E1E] font-[600] mb-[20px] lg:mb-0 lg:w-[500px] lg:text-[36px] '>
                <h1>Please enter the <span className='text-[#A1A1A1] '>sender’s information</span> to proceed</h1>
            </div>
            ...
            <form className='w-[100%] '>
                ...
                <button type='button' className='bg-[#0A089A] p-[20px] flex gap-x-[10px] items-center justify-center w-[100%] text-[#FEFEFE] text-[16px] rounded-[8px] lg:text-[18px]' onClick={handleFormSubmit} >
                    Proceed to Receiver’s Information
                    <Image 
